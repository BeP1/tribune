...
app.get("/api/matches/:date", async (req, res) => {
    ...
    try {
        const promises = LEAGUES.map((leagueId) =>
            fetchData(`${BASE_URL}fixtures?league=${leagueId}&season=2024&timezone=Europe/Kiev&date=${date}`)
    );
...
app.get("/api/league/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const data = await fetchData(`${BASE_URL}standings?season=2024&league=${id}`);
        res.json(data);
    } catch (error) {
...
app.get("/api/club-details/:clubId/:leagueId", async (req, res) => {
    ...
    try {
        const clubDetails = await fetchData(`${BASE_URL}teams/statistics?season=2024&team=${clubId}&league=${leagueId}`);
...
app.get("/api/player-details/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const playerDetails = await fetchData(`${BASE_URL}players?season=2024&id=${id}`);
