...
const MatchDetails = () => {
    ...
    useEffect( () => {
        ...
        MatchesService.MatchDetails(id).then((data) => {
            ...
            if(data[2] && data[2].length > 0) {
                const oddsData = data[2][0].bookmakers.find(bm => bm.name === "Bet365");
                if (oddsData) {
                    const odds = {
                        matchWinner: oddsData.bets.find(bet => bet.name === "Match Winner"),
                        goalsOverUnder: oddsData.bets.find(bet => bet.name === "Goals Over/Under"),
                        bothTeamsToScore: oddsData.bets.find(bet => bet.name === "Both Teams Score")
                    };
    ...
    const team1Stats = statistics[0];
    const team2Stats = statistics[1];
    const date = new Date(fixture.date)

    return (
        <div className="fixture-container">
            <div className="fixture-details">
                <div className="home" onClick={() => goClubDetails(teams.home.id, league.id)}>
                ...
                <div>
                    ...
                    <div className="aditional-details">
                        ...
                        <p>{league.name}, {league.country}, {league.round}</p>
                        <p>Referee: {fixture.referee === null ? "TBD" : fixture.referee}</p>
                        <p>Venue: {fixture.venue.name}, {fixture.venue.city}</p>
                    </div>
                </div>
                <div className="away" onClick={() => goClubDetails(teams.away.id, league.id)}>
            ...
            {statistics && statistics.length > 1 && (
                <div className="fixture-statistics">
                    <div className="teams-header">
                        <div className="team">
                            <img src={team1Stats.team.logo} alt={team1Stats.team.name}/>
                            <p>{team1Stats.team.name}</p>
                        </div>
                        <div className="team">
                            <img src={team2Stats.team.logo} alt={team2Stats.team.name}/>
                            <p>{team2Stats.team.name}</p>
                        </div>
                    </div>
                    <div className="statistics-list">
                        {team1Stats.statistics.map((stat, index) => {
                            const team1Value = stat.value || 0;
                            const team2Value = team2Stats.statistics[index]?.value || 0;

                            const isPercentage = typeof team1Value === "string" && team1Value.includes("%");

                            const team1ValueNumeric = isPercentage ? parseFloat(team1Value) : parseFloat(team1Value) || 0;
                            const team2ValueNumeric = isPercentage ? parseFloat(team2Value) : parseFloat(team2Value) || 0;

                            const total = team1ValueNumeric + team2ValueNumeric;

                            const team1Width = total > 0 ? (team1ValueNumeric / total) * 100 : 50;
                            const team2Width = total > 0 ? (team2ValueNumeric / total) * 100 : 50;
            ...
            <div className="lineups-section">
                ...
                <div className="lineups-container">
                    ...
                    {lineups.map((teamLineup, index) => (
                        <div key={index} className={`team-lineup ${index === 0 ? 'left-team' : 'right-team'}`}>
                            <div className="team-header">
                                <img src={teamLineup.team.logo} alt={teamLineup.team.name} className="team-logo"/>
                                <div>
                                    <h4>{teamLineup.team.name}</h4>
                                    <p>Formation: {teamLineup.formation}</p>
                                </div>
                                <div className="coach-info">
                                    <img src={teamLineup.coach.photo} alt={teamLineup.coach.name}
            ...
            <div className="pre-match-odds">
                ...
                <div className="odds-row">
                    <div className="odds-category">
                        ...
                        <div className="odds-options">
                            {preMatchOdds.matchWinner?.values.map((value, index) => (
                                <div key={index} className={`odds-option ${value.won ? 'highlighted' : ''}`}>
                                    {value.value === "Home" ? "1" : value.value === "Draw" ? "X" : "2"} {value.odd}
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="odds-category">
                        ...
                        <div className="odds-options">
                            {preMatchOdds.goalsOverUnder?.values
                                .filter(val => val.value === "Over 2.5" || val.value === "Under 2.5")
                                .map((value, index) => (
                                    <div key={index} className={`odds-option ${value.won ? 'highlighted' : ''}`}>
                                        {value.value === "Over 2.5" ? "Over" : "Under"} {value.odd}
                                    </div>
                                ))}
                        </div>
                    </div>
                    <div className="odds-category">
                        ...
                        <div className="odds-options">
                            {preMatchOdds.bothTeamsToScore?.values.map((value, index) => (
                                <div key={index} className={`odds-option ${value.won ? 'highlighted' : ''}`}>
